...
function localToDb(user) {
  return {
    ...
    saved_products: user.savedProducts || [],
...
function mergeLocalToCloud(local, cloud) {
  return {
    ...
    savedProducts: (cloud.savedProducts?.length >= local.savedProducts?.length) ? cloud.savedProducts : local.savedProducts || [],
...
export async function loadFromSupabase(userId) {
  ...
  // Preserve local savedProducts if cloud doesn't have them
  if ((localUser.savedProducts || []).length > 0 && (merged.savedProducts || []).length === 0) {
    merged.savedProducts = localUser.savedProducts;
  }
...
export function saveProduct(product) {
  ...
  const exists = user.savedProducts.some((p) => p.name === product.name && p.affiliate_link === product.affiliate_link);
  if (exists) return { saved: false, user };
  user.savedProducts.push({
    name: product.name,
    price: product.price,
    affiliate_link: product.affiliate_link,
    why_recommended: product.why_recommended || null,
...
export function removeSavedProduct(index) {
  const user = getUser();
  if (!user.savedProducts) return user;
  user.savedProducts.splice(index, 1);
  saveUser(user);
  return user;
}

export function isProductSaved(product) {
  const user = getUser();
  return (user.savedProducts || []).some((p) => p.name === product.name && p.affiliate_link === product.affiliate_link);
}
